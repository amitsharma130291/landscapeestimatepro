...
export default function PriceListBuilderIsland() {
  ...
  const [minimumProject, setMinimumProject] = useState<number | "">(500);
  const [rows, setRows] = useState<PriceRow[]>([
    { id: crypto.randomUUID(), service: "Mulch installation", rate: 95, unit: "yd³" },
    { id: crypto.randomUUID(), service: "Topsoil installation", rate: 85, unit: "yd³" },
    { id: crypto.randomUUID(), service: "Edging", rate: 2.25, unit: "linear ft" },
    { id: crypto.randomUUID(), service: "Shrub installation", rate: 65, unit: "each" },
  ]);
    ...
    <div>
      ...
      <Card className="mt-6" padded={false}>
        <div className="flex items-center justify-between p-5 pb-0 sm:p-6 sm:pb-0">
          ...
          <Button type="button" variant="ghost" size="sm" onClick={() => setRows((prev) => [...prev, { id: crypto.randomUUID(), service: "", rate: 0, unit: "each" }])}>
              ...
              {rows.map((row, index) => (
                <tr key={row.id} className="border-b border-border last:border-b-0">
                  ...
                  <td className="px-3 py-2.5">
                    ...
                    <NumberInput id={`${idPrefix}-rate-${index}`} value={row.rate} onValueChange={(v) => updateRow(row.id, { rate: v })} className="w-28" />
                  </td>
                  <td className="px-3 py-2.5">
        ...
        <div className="border-t border-border p-5 sm:p-6">
          <Field label="Minimum project price" htmlFor={`${idPrefix}-minimum`} hint="The lowest price you'll quote for any job, regardless of size">
            <div className="relative max-w-[10rem]">
              ...
              <NumberInput id={`${idPrefix}-minimum`} value={minimumProject} onValueChange={setMinimumProject} className="pl-7" />
            </div>
          </Field>
        </div>
      </Card>
      ...
      <div className="mt-6 rounded-2xl border border-lime-surface bg-mint p-5 sm:p-6">
        ...
        <p className="mt-1.5 text-sm text-mint-ink/80">
          ...
          below {formatCurrency(safe(minimumProject === "" ? 0 : minimumProject))} minimum or below target.
        </p>
